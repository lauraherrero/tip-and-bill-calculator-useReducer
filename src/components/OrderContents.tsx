...
export const OrderContents = ({ order, dispatch }: OrderItemProps) => {
      ...
      <div className="space-y-3 mt-10">
        {order.map(item =>
          <div className="flex justify-between items-center border-t border-gray-200 py-5 last-of-type:border-b" key={item.id}>
            ...
            <button className="bg-red-600 h-8 w-8 rounded-full text-white font-black" onClick={() => dispatch({type: 'REMOVE_ITEM', payload: {id: item.id}})}>X</button>
          </div>)}
